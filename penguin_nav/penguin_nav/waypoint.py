...
class Waypoint:
    ...
    def right(self):
        return self._solve(-self.right_tolerance)
...
class Plan:
    ...
    def to_msg(
        ...
        return (
            ...
            [p.right_tolerance for p in self.waypoints],
        )
...
class Loader:
    ...
    left_tolerance 及び right_tolerance は、点の位置を調整量である。
    ...
    def __init__(self, x="x", y="y", yaw="yaw", yaw_deg="yaw_deg", action="action"):
        ...
        self._action = action
        self._left_tolerance = "left_tolerance"
        self._right_tolerance = "right_tolerance"
    ...
    def _convert(self, df: pd.DataFrame) -> List[Waypoint]:
        return [
            Waypoint(
                ...
                left_toralence=row[self._left_tolerance],
                right_tolerance=row[self._right_tolerance],
            )
    ...
    def _load_and_concat(self, files: List[str]) -> pd.DataFrame:
        def _read(f):
            ...
            # tolerance
            if self._left_tolerance not in df.columns:
                df[self._left_tolerance] = 0.0
            df.loc[df[self._left_tolerance].isna(), [self._left_tolerance]] = 0.0

            if self._right_tolerance not in df.columns:
                df[self._right_tolerance] = 0.0
            df.loc[df[self._right_tolerance].isna(), [self._right_tolerance]] = 0.0
